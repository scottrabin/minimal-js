...
define({
	...
	max: function(compare) {
		if (!compare || compare.length === 1) {
			...
			var valueFn = compare || function(i) { return i; };
		...
		return this.reduce(function(max, value) {
			return (compare(value, max) > 0 ? value : max);
		}, this.first());
	},
	/**
	...
	min: function(compare) {
		...
		return this.reduce(function(min, value) {
			return (compare(value, min) < 0 ? value : min);
		}, this.first());
	},
	/**
	...
	range: function(compare) {
		...
		this.forEach(function(value) {
			if (compare(value, range[0]) < 0) {
				range[0] = value;
			} else if (compare(value, range[1]) > 0) {
				range[1] = value;
			}
		});
		return range;
